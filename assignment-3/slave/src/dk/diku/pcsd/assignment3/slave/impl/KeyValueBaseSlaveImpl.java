...
public class KeyValueBaseSlaveImpl extends KeyValueBaseReplicaImpl implements KeyValueBaseSlave<KeyImpl, ValueListImpl> {
	...
	public void logApply(LogRecord record) {
		try{
			if (record.getMethodName().equals("init")){
				...
				ObjectInputStream in = new ObjectInputStream(new FileInputStream(getIndexFile()));
	...
	private File getIndexFile(){
		...
		return new File(tmpDir + getClass().getPackage().getName().replaceAll("\\.", Matcher.quoteReplacement(File.separator)) + File.separator + uid + ".ind");
	}
