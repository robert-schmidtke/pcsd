...
/**
 * Custom serializer and deserializer. Uses ByteBuffer for better performance.
 * Can be used to serialize ValueListImpls containing ValueImpls containing
 * Strings or any Java primitives.
 * Stores the result of the toString() method for other objects.
 */
public class ValueSerializerImpl implements ValueSerializer<ValueListImpl> {

	// type markers
	private static final byte TYPE_BYTE = 0, TYPE_BOOLEAN = 1,
			TYPE_CHARACTER = 2, TYPE_SHORT = 3, TYPE_INTEGER = 4,
			TYPE_LONG = 5, TYPE_FLOAT = 6, TYPE_DOUBLE = 7, TYPE_STRING = 8;
	...
	private int getSerializedLength(Object value) {
		...
		return ((String) value).toString().getBytes().length + 1 + 4; // +
																		// length
																		// field
	}
